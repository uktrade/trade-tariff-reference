...
class Hierarchy:
    ...
    def get_hierarchy(self, direction):
        ...
        for commodity in ar_goods_nomenclatures:
            if (
                    (commodity.goods_nomenclature_item_id == self.goods_nomenclature_item_id)
                    and (commodity.productline_suffix == self.productline_suffix)
            ):
        ...
        for i in range(my_index, 0, -1):
            ...
            if (
                    (t.number_indents < temp_indent)
                    or (
                    (t.goods_nomenclature_item_id == self.goods_nomenclature_item_id)
                    and (t.productline_suffix == self.productline_suffix)
                    )
            ):
